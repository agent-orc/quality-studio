...
Re-verified 2026-08-27 on this base (`f7557fb`) against two real registered
repositories: this checkout (327 tracked files) and a second Agent Studio
worktree checkout (328 tracked files), both served by a locally running
`QualityStudio.Api`.

| Measurement | Cold | Warm |
| --- | ---: | ---: |
| Fresh-process prewarm, this repository | 1,835.97 ms | 8.01 ms |
| Fresh-process prewarm, second repository | 1,019.45 ms | 7.76 ms |
| `/project` after a real working-tree change (`git-status`/`scan`/`review-meta-discovery`/`projection`) | 6.50 + 652.96 + 2.75 + 145.53 ms | 6.4–7.3 ms (`git-status` only; the other three phases are 0 ms) |
| Operator-equivalent parallel `project` + `tree` switch | 690 ms | 32–60 ms |

The warm numbers match the 2026-08-24 measurement within noise, confirming
the cache still activates on this base. The remembered-repository and
API-offline behaviour added in this delivery were exercised end to end with
a real browser against this same API; see
`results/qs-78-evidence.json` and its screenshots.
