...
**The engineer room of the Agent Orchestrator universe: agent-driven, layered code reviews with quality truth kept outside the analysed checkout.**
...
### 2. Review metadata follows the code without modifying it

Every reviewed unit gets a small structured JSON meta file in Quality Studio's
application-owned data root. The data root mirrors the repository-relative folder
layout, so metadata remains associated with its feature without being written into
the Git checkout:
...
no longer applies. Quality Studio owns runtime history; exports provide deliberate,
portable review artifacts without making routine analysis dirty the checkout.
...
The running studio treats every registered checkout as read-only. Findings, reports,
usage ledgers, run state, coverage snapshots, boundary inventories, project inputs,
and review sidecars are read and written below a project-specific shadow tree. The
default is `%LOCALAPPDATA%/QualityStudio/projects/<project-id>/` on Windows (the
platform local-application-data equivalent elsewhere). Configure the base directory
with `QualityStudio:DataRoot` or `QUALITY_STUDIO_DATA_ROOT`.

No `.quality/**` runtime artifact is meant to be versioned. `.gitignore` ignores root
and nested `.quality/` directories. Versioned deliverables are explicit exports, for
example `quality report . --format sarif --output quality-report.sarif`; the operator
chooses an output path and commits it intentionally if required.

After upgrading an existing installation, invoke
`POST /api/repos/<project-id>/migrate-quality-data` once. The backend moves root and
per-folder `.quality` trees into the matching project shadow tree and records a
migration marker. If different data already exists at a destination, migration stops
instead of overwriting it. See [the data-root operations contract](docs/operations/data-root.md).
...
The stable result is written to the project's external `.quality/boundaries/inventory.json`. See
...
Change truth is retained under the project's external `.quality/changes/`. See
...
identity both with the review truth and in a project data-root append-only ledger.
...
Run-scoped exports render the exact terminal snapshot captured under the project's
external `.quality/reports/runs/`; they do not re-read mutable review sidecars. CI gates
