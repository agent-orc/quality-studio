...
The required gate pins .NET 10.0.301 and Node 22.23.1, excludes machine-bound
timing assertions from the portable .NET lane, installs the locked frontend
dependency graph, and builds the production Angular application under the
unchanged 480 kB initial-bundle limit. It also provisions the Chromium revision
declared by the frontend lock file before Angular tests and resolves the pinned,
verified Gitleaks binary before the security scan.

Run the equivalent checks locally from a clean checkout:
...
Set `CHROME_NO_SANDBOX=1` only on a controlled Linux runner where the Chromium
sandbox is unavailable. Host-integration, coverage-ratchet, and release-canary
checks are separate baseline slices and do not replace this portable gate.
