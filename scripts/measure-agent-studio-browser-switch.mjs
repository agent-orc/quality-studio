...
try {
  ...
  try {
    ...
    await switchRepository(page, 'Quality Studio');
    await page.locator('.api-offline-bar').filter({ hasText: 'API unreachable' }).waitFor({ state: 'visible' });
    await page.screenshot({ path: resolve(resultsRoot, 'agent-studio-api-down-notice.png') });

    await page.unroute('**/api/**');
    await page.locator('.api-offline-bar').getByRole('button', { name: 'Retry' }).click();
    await page.locator('.api-offline-bar').waitFor({ state: 'detached' });
