...
const apiDll = process.env.QS_API_DLL ||
  resolve(repositoryRoot, 'backend/QualityStudio.Api/bin/Release/net10.0/QualityStudio.Api.dll');
...
try {
  ...
  const restoredRuns = [];
  for (let index = 0; index < restoredStartupSamples; index++) {
    const process = await startApi();
    const firstSwitch = await measureSwitch(process.url, 'v2');
    await waitForEvent(process, 'qs.repository.prewarm', 90_000);
    const prewarm = parseEvent(process.lines, 'qs.repository.prewarm');
    const warmSwitch = await measureSwitch(process.url, 'v2');
    restoredRuns.push({
      processToHealthMs: round(process.healthMs),
      processToProjectAndTreeMs: round(process.healthMs + firstSwitch.durationMs),
      firstSwitch,
      warmSwitch,
      prewarm,
    });
  ...
  const restoredLegacyProcess = await startApi();
  const restoredLegacySwitch = await measureSwitch(restoredLegacyProcess.url);
  await stop(restoredLegacyProcess.child);
  const restoredStartup = restoredRuns[0];
  ...
  const result = {
    ...
    before: {
      source: 'QS-59 performance dossier, refreshed 2026-08-12, Agent Taskboard / Agent Studio repository at 9af1a848',
      coldProcessToProjectAndTreeMedianMs: 10_004.99,
    ...
    after: {
      ...
      restoredStartup: {
        ...restoredStartup,
        legacyCompatibilitySwitch: restoredLegacySwitch,
      },
      restoredStartups: {
        samples: restoredRuns,
        processToHealth: summarize(restoredRuns.map(sample => sample.processToHealthMs)),
        processToProjectAndTree: summarize(restoredRuns.map(sample => sample.processToProjectAndTreeMs)),
      },
...
async function measureSwitch(url, treeVersion = 'v1') {
  const started = performance.now();
  const treeUrl = treeVersion === 'v2'
    ? `${url}/api/tree/v2?limit=500`
    : `${url}/api/tree?path=`;
  const [project, tree] = await Promise.all([
    measureRequest(`${url}/api/project`),
    measureRequest(treeUrl),
  ]);
  return { treeVersion, durationMs: round(performance.now() - started), project, tree };
}
