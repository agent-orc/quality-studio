...
export function assertNpmAudit(document, source = 'npm audit') {
  if (document?.auditReportVersion !== 2 || !document.metadata?.vulnerabilities) {
    throw new Error(`${source} is not a recognized npm audit result.`);
  }
  const { high = 0, critical = 0 } = document.metadata.vulnerabilities;
  if (!Number.isInteger(high) || !Number.isInteger(critical)) {
    throw new Error(`${source} has invalid vulnerability counts.`);
  }
  if (high + critical > 0) {
    throw new Error(`${source} contains ${high} high and ${critical} critical production vulnerabilities.`);
  }
}
...
export function assertSbom(document, source = 'SBOM') {
  if (document?.spdxVersion !== 'SPDX-2.2' || !Array.isArray(document.packages)) {
    throw new Error(`${source} is not a recognized SPDX 2.2 document.`);
  }
  if (document.packages.length < 2) {
    throw new Error(`${source} contains no detected dependency packages.`);
  }
}
...
function visit(value, path, vulnerabilities) {
  if (Array.isArray(value)) {
    for (let index = 0; index < value.length; index += 1) {
      visit(value[index], `${path}[${index}]`, vulnerabilities);
    }
    return;
  }
...
async function main(args) {
  ...
  if (!command || paths.length === 0) {
    throw new Error('Usage: security-policy.mjs <npm|nuget|sbom|config> <json-path> [json-path...]');
  }
  for (const path of paths) {
    const document = JSON.parse(await readFile(path, 'utf8'));
    if (command === 'npm') assertNpmAudit(document, path);
    else if (command === 'nuget') assertNugetAudit(document, path);
    else if (command === 'sbom') assertSbom(document, path);
