...
export async function runDotnetLane(arguments_) {
  ...
  for (const project of projects) {
    ...
    if (options.coverage || options.coverageRoot) {
      ...
      if (options.coverageRoot) {
        ...
        args.push('--results-directory', resultsDirectory);
      }
      // Without an explicit results directory the reports stay in the project's own
      // TestResults folder, which is where the repository coverage validator looks.
      args.push('--', 'DataCollectionRunSettings.DataCollectors.DataCollector.Configuration.Format=cobertura');
    }
...
function parseArguments(arguments_) {
  ...
  const options = {
    lane, configuration: 'Release', noBuild: false, listOnly: false, coverage: false, coverageRoot: null,
  };
