...
function stripIgnoredContent(content) {
  ...
  // truth, not a violation; comments are prose rather than declarations. Replace both with spaces
  // so match indexes still map to the original line and column.
  return content
...
function collectTokenLiterals(content) {
  const values = new Map();
  for (const declaration of content.matchAll(TOKEN_DECLARATION)) {
    const tokenName = declaration[0].slice(0, declaration[0].indexOf(':')).trim();
    for (const match of [...findMatches(declaration[1], HEX_COLOR), ...findMatches(declaration[1], RAW_PX)]) {
      const key = normalizeLiteral(match.value);
...
async function main() {
  ...
  const results = [];
  const tokenSource = join(repositoryRoot, 'frontend', 'src', 'styles.css');
  let tokenLiterals = new Map();
  try {
    tokenLiterals = collectTokenLiterals(await readFile(tokenSource, 'utf8'));
  } catch (error) {
    if (error?.code !== 'ENOENT') throw error;
  }
  const tokenOccurrences = new Map(); // stylesheet + token-backed literal -> locations, for QS-NG-003
  const literalOccurrences = new Map(); // non-token raw literal -> Set(relativePath), for QS-NG-004
  ...
  for (const file of files) {
    ...
    for (const match of [...findMatches(scanned, HEX_COLOR), ...findMatches(scanned, RAW_PX)]) {
      const normalized = normalizeLiteral(match.value);
      const matchingTokens = tokenLiterals.get(normalized);
      if (matchingTokens) {
        const { line, column } = lineAndColumnAt(raw, match.index);
        const key = `${relativePath}\0${normalized}`;
        const bucket = tokenOccurrences.get(key) ?? {
          display: match.value,
          tokens: new Set(matchingTokens),
          locations: [],
        };
        ...
        tokenOccurrences.set(key, bucket);
      } else {
        const bucket = literalOccurrences.get(normalized) ?? { display: match.value, files: new Set() };
        bucket.files.add(relativePath);
        literalOccurrences.set(normalized, bucket);
      }
    }
  }

  for (const { display: literal, tokens: matchingTokens, locations } of tokenOccurrences.values()) {
    results.push({
      ruleId: 'QS-NG-003',
      level: 'warning',
      message: {
        text: `Raw literal '${literal}' occurs ${locations.length} time(s) and duplicates ${[...matchingTokens].sort().join('/')} from frontend/src/styles.css.`,
      },
      locations,
    });
  }

  for (const { display: literal, files } of literalOccurrences.values()) {
