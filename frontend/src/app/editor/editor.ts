...
export class Editor {
  ...
  segmentedLine(line: number, text: string): SegmentedSpan[] {
    ...
    if (!ranges?.length) return this.tokensForLine(line, text).map(token => ({ ...token, state: 'plain' }));
    const selected = this.selectedFinding();
    return segmentLineTokens(this.tokensForLine(line, text), line, ranges, selected ? selected.fingerprint ?? selected.id : null);
  }
