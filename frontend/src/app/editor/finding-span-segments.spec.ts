...
describe('segmentFindingSpans', () => {
  ...
  it('retains a zero-width end-of-line anchor as a programmatic segment', () => {
    const selected = finding('g', 1, 4, 1, 4);
    const segment = segmentFindingSpans([{ text: 'abc', kind: 'plain' }], 1, 'src/A.cs', [selected], selected.fingerprint).at(-1)!;

    expect(segment.text).toBe('');
    expect(segment.endOfLine).toBeTrue();
    expect(segment.startColumn).toBe(4);
    expect(segment.endColumn).toBe(4);
