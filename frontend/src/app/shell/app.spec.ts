...
import { App } from './app';
import { ReviewFinding } from '../core/models/contracts';
...
describe('App shell URL state', () => {
  ...
  it('replaces the workspace with an actionable API outage and restores it after reconnecting', () => {
    app.api.connectionState.set('offline');
    app.api.connectionError.set('The API is not reachable.');
    fixture.detectChanges();

    const notice = fixture.nativeElement.querySelector('.api-unavailable') as HTMLElement;
    const workspace = fixture.nativeElement.querySelector('.workspace') as HTMLElement;
    expect(notice).withContext('the outage occupies the workspace, not a small header label').not.toBeNull();
    expect(notice?.textContent).toContain('API unavailable');
    expect(notice?.textContent).toContain('Start the API to continue');
    expect(workspace.hasAttribute('inert')).toBeTrue();
    expect(getComputedStyle(workspace).display).toBe('none');
    expect((fixture.nativeElement.querySelector('.repository-trigger') as HTMLButtonElement).disabled).toBeTrue();

    const retry = spyOn(app.api, 'retryConnection').and.resolveTo();
    (notice.querySelector('[data-action="retry-api"]') as HTMLButtonElement).click();
    expect(retry).toHaveBeenCalledTimes(1);

    app.api.connectionState.set('connecting');
    fixture.detectChanges();
    expect(notice.textContent).toContain('Connecting to API');
    expect((notice.querySelector('[data-action="retry-api"]') as HTMLButtonElement).disabled).toBeTrue();

    app.api.connectionState.set('live');
    fixture.detectChanges();
    expect(fixture.nativeElement.querySelector('.api-unavailable')).toBeNull();
