...
describe('ReviewHistory', () => {
  ...
  it('renders honest legacy usage summaries without enabling detail or compare', async () => {
    const fixture = TestBed.createComponent(ReviewHistory);
    const legacy: ReviewHistorySummary = {
      ...summary('legacy'), createdAt: null, startedAt: null, finishedAt: null, attempt: null,
      outcome: 'legacy-usage-only', operations: 0, findings: 0, quality: null,
      model: 'gpt-5.1', provenance: 'legacy-usage-only',
      spend: { tokens: { inputTokens: 25, outputTokens: 5, cachedInputTokens: 3, reasoningOutputTokens: null, durationMs: 40 }, cost: null, currency: null, priceStatus: 'unavailable' },
    };

    const opening = fixture.componentInstance.toggle();
    http.expectOne('/api/repos/default/review/history?limit=20').flush({ runs: [legacy], nextCursor: null });
    await opening;
    fixture.detectChanges();

    expect(fixture.nativeElement.querySelector('.history-main').disabled).toBeTrue();
    expect(fixture.nativeElement.querySelector('.history-row').textContent).toContain('25 input · 5 output');
    expect(fixture.componentInstance.canSelect(legacy)).toBeFalse();
    await fixture.componentInstance.selectRun(legacy);
