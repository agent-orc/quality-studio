...
export class QualityApi {
  ...
  async loadTree(repositoryId = this.selectedRepositoryId(), waitForDetails = true): Promise<void> {
    ...
    try {
      const nodes = await this.loadTreeLevel(base, null, repositoryId);
  ...
  async loadTreeChildren(node: TreeNode, repositoryId = this.selectedRepositoryId()): Promise<void> {
    ...
    const request = (async () => {
      try {
        const children = await this.loadTreeLevel(this.repositoryApiBase(repositoryId), node.id, repositoryId);
  ...
  private async loadTreeLevel(base: string, parentId: string | null, repositoryId: string): Promise<TreeNode[]> {
