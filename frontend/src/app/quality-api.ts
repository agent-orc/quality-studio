...
export class QualityApi {
  ...
  async mutateFindingAssessment(request: FindingAssessmentMutationRequest): Promise<void> {
    await firstValueFrom(this.http.post(`${this.repositoryApiBase()}/findings/assessment`, request));
    await this.loadFile(request.path);
    console.info(JSON.stringify({ event: 'qs.finding.assessment-mutated', fingerprint: request.fingerprint, path: request.path, assessment: request.assessment, resolution: request.resolution }));
  }

  async suppressFindingExact(request: { path: string; kind: ReviewKind; fingerprint: string; author: string; reason: string; expiresAt?: string | null; expectedRevision: number }): Promise<void> {
    await firstValueFrom(this.http.post(`${this.repositoryApiBase()}/findings/suppressions/exact`, request));
    await this.loadFile(request.path);
    console.info(JSON.stringify({ event: 'qs.finding.suppressed', fingerprint: request.fingerprint, path: request.path }));
  }

  previewFindingSuppression(rule: FindingSuppressionRule): Promise<FindingSuppressionPreview> {
    return firstValueFrom(this.http.post<FindingSuppressionPreview>(`${this.repositoryApiBase()}/findings/suppressions/preview`, { rule }));
  }

  async saveFindingSuppression(rule: FindingSuppressionRule, expectedRevision: number, pathToReload: string): Promise<FindingSuppressionPreview> {
    const result = await firstValueFrom(this.http.put<FindingSuppressionPreview>(
      `${this.repositoryApiBase()}/findings/suppressions/${encodeURIComponent(rule.id)}`, { rule, expectedRevision }));
    await this.loadFile(pathToReload);
    console.info(JSON.stringify({ event: 'qs.finding.suppression-saved', ruleId: rule.id, matchCount: result.matchCount }));
    return result;
  }
