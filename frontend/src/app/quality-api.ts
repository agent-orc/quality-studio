...
const demoMeta: ReviewMetaDocument[] = [
  { reviewedAt: '2026-07-11T16:20:00.000Z', kind: 'code', reviewer: { agent: 'quality-reviewer', model: 'gpt-5' }, grade: { score: 91, band: 'A', rationale: 'Clear request boundaries and consistent error handling.' }, summary: 'The API entry point is compact and readable. Two localized diagnostic and boundary gaps remain.', findings: [
    { id: 'route-timing', ruleId: 'dotnet-api-safety', aspect: 'observability', severity: 'low', title: 'File route has no timing event', description: 'The user-visible file read is not timed, making slow repository access difficult to diagnose.', recommendation: 'Record a structured duration for the file-read path.', evidence: 'The route awaits File.ReadAllTextAsync and returns without a timing log.', locations: [{ path: 'src/QualityStudio.Api/Program.cs', range: { start: { line: 20, column: 19 }, end: { line: 20, column: 51 } } }] },
    { id: 'direct-file-read', ruleId: 'repository-boundary', aspect: 'correctness', severity: 'medium', title: 'File read bypasses repository access', description: 'The route passes its request path directly to the filesystem instead of the configured repository boundary.', recommendation: 'Resolve and confine the path through RepositoryAccess before reading it.', locations: [{ path: 'src/QualityStudio.Api/Program.cs', range: { start: { line: 20, column: 25 }, end: { line: 20, column: 51 } } }] },
  ] },
