...
export class QualityApi {
  ...
  async loadRepositories(preferredId?: string | null): Promise<void> {
    ...
    try {
      ...
      const lastRepositoryId = this.readLastRepositoryId();
  ...
  /** Re-runs the calls the shell needs to recover after the API was unreachable. */
  ...
  private writeLastRepositoryId(id: string): void {
    try { localStorage.setItem(LAST_REPOSITORY_STORAGE_KEY, id); } catch { /* storage unavailable */ }
  }

  /** Distinguishes a fully unreachable API (network/connection failure) from a reachable
   *  server returning an error, so the shell can show "offline" only when nothing answered. */
