...
export class ReviewPanel {
  ...
  async toggleIgnoreList(): Promise<void> {
    if (this.ignoreListOpen()) {
      this.ignoreListOpen.set(false);
      return;
    }
    this.ignoreListOpen.set(true);
    this.suppressionStatus.set('Loading ignore list…');
    try {
      this.suppressionDocument.set(await this.api.loadFindingSuppressions());
      this.suppressionStatus.set('');
    } catch (error) { this.suppressionStatus.set(this.api.errorMessage(error)); }
  ...
  async removeSuppression(rule: FindingSuppressionRule): Promise<void> {
    this.suppressionStatus.set(`Removing ${rule.id}…`);
    try {
      this.suppressionDocument.set(await this.api.removeFindingSuppression(
        rule.id, this.suppressionDocument().revision));
      this.suppressionStatus.set('Ignore-list rule removed; matching observations are visible again.');
    } catch (error) { this.suppressionStatus.set(this.api.errorMessage(error)); }
  }

  suppressionMatchLabel(rule: FindingSuppressionRule): string {
    const match = rule.match;
    const fields = [
      match.fingerprint ? `exact ${match.fingerprint.slice(7, 19)}…` : null,
      match.ruleId ? `rule ${match.ruleId}` : null,
      match.pathPattern ? `path ${match.pathPattern}` : null,
      ...(match.reviewKinds ?? []).map(kind => `review ${kind}`),
      ...(match.sourceKinds ?? []).map(kind => `source ${kind}`),
    ];
    return fields.filter((field): field is string => !!field).join(' · ');
  }
