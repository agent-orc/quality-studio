...
export class ReviewPanel {
  ...
  readonly comparisonCandidates = computed(() => this.comparableTrend().filter(candidate =>
    this.comparableTrend().some(baseline => baseline.runId !== candidate.runId &&
      Date.parse(baseline.finishedAt) < Date.parse(candidate.finishedAt))));
  ...
  async openComparison(): Promise<void> {
    ...
    let candidate = this.comparisonCandidates().find(point => point.runId === this.selectedRunId()) ??
      this.comparisonCandidates()[0];
    if (!candidate) return;
  ...
  closeComparisonFromKeyboard(): void {
    if (this.comparisonOpen()) this.closeComparison();
  }
