...
describe('ReviewPanel session flow', () => {
  ...
  const api = {
    ...
    removeFindingSuppression: jasmine.createSpy('removeFindingSuppression'),
  ...
  beforeEach(async () => {
    ...
    for (const spy of [api.mutateFindingState, api.mutateFindingAssessment, api.mutateFindingResolution,
      api.loadFindingSuppressions, api.previewFindingSuppression, api.addFindingSuppression, api.removeFindingSuppression,
    ...
    api.removeFindingSuppression.and.resolveTo({ schemaVersion: 1, revision: 2, rules: [] });
  ...
  it('loads and removes repository-backed rules through the visible Ignore list', async () => {
    const rule = {
      id: 'exact-a', enabled: true, match: { fingerprint: openFinding.fingerprint }, effect: 'suppress' as const,
      reason: 'Known exact noise.', author: 'Reviewer', createdAt: '2026-08-11T08:00:00Z', expiresAt: null,
    };
    api.loadFindingSuppressions.and.resolveTo({ schemaVersion: 1, revision: 1, rules: [rule] });

    await component.toggleIgnoreList();
    fixture.detectChanges();
    expect(api.loadFindingSuppressions).toHaveBeenCalled();
    expect(fixture.nativeElement.querySelector('[aria-label="Finding ignore list"]').textContent).toContain('Known exact noise.');

    await component.removeSuppression(rule);
    expect(api.removeFindingSuppression).toHaveBeenCalledWith('exact-a', 1);
    expect(component.suppressionDocument().rules).toEqual([]);
    expect(component.suppressionStatus()).toContain('visible again');
  });
