...
describe('QualityApi', () => {
  ...
  it('restores the last active repository only on the first registry load', async () => {
    ...
    // A later registry refresh keeps the current selection instead of restoring stale state.
  ...
  it('lets an explicit repository route override the remembered repository', async () => {
    localStorage.setItem('qs-last-repository', 'beta');
  ...
  it('remembers a repository when the operator switches', async () => {
    ...
    spyOn(api as unknown as { loadRepositoryDetails(id: string): Promise<void> }, 'loadRepositoryDetails').and.resolveTo();

    await api.selectRepository('alpha');

    expect(api.selectedRepositoryId()).toBe('alpha');
    expect(localStorage.getItem('qs-last-repository')).toBe('alpha');
  });

  it('falls back to the default repository when the remembered repository is gone', async () => {
