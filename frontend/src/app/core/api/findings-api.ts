 ...
 *
 * A failed lookup produces a `fileError`, never someone else's content or demonstration data.
 */
...
export class FindingsApi {
  ...
  async loadFile(path: string): Promise<void> {
    ...
    const repositoryId = this.context.selectedRepositoryId();
    const current = () => generation === this.fileRequestGeneration && repositoryId === this.context.selectedRepositoryId();
    this.loading.set(true);
    try {
      const file = await firstValueFrom(this.http.get<FileDocument>(`${this.context.repositoryApiBase(repositoryId)}/file`, { params: { path } }));
      if (!current()) return;
      this.file.set(file);
