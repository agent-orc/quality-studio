...
for (const capture of [
  ...
  const keyboardClosed = await page.locator('.comparison-workbench').count() === 0;
  if (!keyboardClosed) throw new Error(`${capture.name}: Escape did not close comparison`);
  evidence.push({ ...capture, fileName, keyboardOpened: true, keyboardClosed, audit });
  await page.close();
}
