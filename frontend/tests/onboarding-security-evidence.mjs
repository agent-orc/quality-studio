...
await page.screenshot({ path: join(output, 'onboarding-before-checks.png'), fullPage: true });

await page.getByRole('button', { name: 'Run onboarding checks' }).click();
await page.getByText('Review quarantined', { exact: true }).waitFor();
await page.screenshot({ path: join(output, 'onboarding-after-quarantine.png'), fullPage: true });
...
console.log(JSON.stringify({
  output,
  screenshots: [
    'onboarding-before-checks.png',
    'onboarding-after-quarantine.png',
    'onboarding-after-advisories.png',
  ],
}, null, 2));
