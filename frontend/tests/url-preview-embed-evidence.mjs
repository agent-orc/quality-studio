...
const dossierOutput = process.env.QS_DOSSIER_ASSETS ? resolve(process.env.QS_DOSSIER_ASSETS) : null;
...
await page.waitForFunction(
  (previousCount) => window.__acceptedNavigation?.length > previousCount,
  initial.count,
);
...
if (navigated.iframeSrc !== initial.iframeSrc)
  throw new Error('Receiver remounted the iframe source');
if (navigated.frameLoadCount !== initial.frameLoadCount)
  throw new Error('Embedded navigation reloaded the iframe');
await capture(
  page,
...
async function readAndAssertLastMessage(browserPage, expected) {
  ...
  if (reported.origin !== state.eventOrigin)
    throw new Error('Reported URL origin does not match event.origin');
  if (
    reported.searchParams.get('repo') !== expected.repository ||
