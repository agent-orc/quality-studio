...
const output = resolve(process.env.JOB_RESULTS_DIR ?? process.argv[2] ?? 'evidence');
...
const meta = {
  reviewedAt: '2026-08-11T10:00:00.000Z', kind: 'code', reviewer: { agent: 'evidence-runner', model: 'deterministic' },
  grade: { score: 88, band: 'B', rationale: 'Exact-span evidence fixture.' }, summary: 'Two findings intersect on the Model parameter line.',
  findings: [
    {
      ...
      locations: [{ path, range: { start: { line: 8, column: 5 }, end: { line: 8, column: 17 } } }],
    },
    {
      ...
      locations: [{ path, range: { start: { line: 8, column: 13 }, end: { line: 8, column: 25 } } }],
    },
  ],
};
...
for (const theme of ['dark', 'light']) {
  ...
  await page.route(/\/api\/(?:repos\/[^/]+\/)?file(?:\?|$)/, route => route.fulfill({
    ...
    body: JSON.stringify({ path, content: source, metaDocuments: [meta], sizeBytes: Buffer.byteLength(source), lineEnding: 'lf', encoding: 'utf-8' }),
  }));
  ...
  if (selectedText !== 'string? ') throw new Error(`${theme}: expected selected remainder "string? ", got "${selectedText}"`);
  const overlapText = await page.locator('.tok-overlap').first().textContent();
  if (overlapText !== 'Model') throw new Error(`${theme}: expected overlap "Model", got "${overlapText}"`);
