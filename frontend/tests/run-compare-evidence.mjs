...
const label = process.env.QS_EVIDENCE_LABEL ?? 'after';
...
const run = {
  id: 'review-evidence-qs83-b', repositoryId: 'default', path: 'src/QualityStudio.Api/Program.cs', level: 'file', kind: 'code',
  model: 'claude-opus-4-8', thinkingLevel: 'high', cliType: 'codex', state: 'done', totalFiles: 1, completedFiles: 1,
  failedFiles: 0, skippedFiles: 0, createdAt: '2026-08-27T08:00:00Z', startedAt: '2026-08-27T08:00:01Z',
  finishedAt: '2026-08-27T08:00:05Z', files: [{ path: 'src/QualityStudio.Api/Program.cs', state: 'done', startedAt: '2026-08-27T08:00:01Z', finishedAt: '2026-08-27T08:00:05Z', error: null }],
...
const report = {
  ...
  observations: [{ unitId: 'program', level: 'file', path: run.path, outcome: 'done', producedByRun: true,
    ...
    capturedAt: '2026-08-27T08:00:05Z', reviewedHash: hash('c'), providerRunId: 'provider-evidence',
  ...
  delta: { status: 'available', priorRunId: 'review-evidence-qs83-a', reason: null, new: [finding.fingerprint], persisting: [], resolved: [], stateChanged: [] },
...
const trend = { points: [
  ...
  { runId: 'review-evidence-qs83-a', revision: 1, finishedAt: '2026-08-20T08:00:00Z', state: 'done', completeness: 'complete', comparable: true, comparisonReason: null, score: 74, grade: 'C', activeFindings: 2, newFindings: 0, persistingFindings: 1, resolvedFindings: 0, stateChangedFindings: 0, reviewed: 1, reusedFresh: 0, failed: 0, skipped: 0, inputTokens: 540, outputTokens: 110, cost: null, currency: null },
], nextCursor: null };
const comparison = {
  fromRunId: 'review-evidence-qs83-a', toRunId: run.id,
  comparabilityLabels: ['model-changed'],
  delta: { status: 'available', priorRunId: 'review-evidence-qs83-a', reason: null, new: [finding.fingerprint], persisting: [hash('e')], resolved: [hash('f')], stateChanged: [] },
};
...
async function fulfillApi(route) {
  ...
  else if (path === '/api/models') body = { schemaVersion: 1, policyVersion: 'evidence', evidenceAsOfDate: '2026-08-27', sourceRepository: 'fixture', sourceCommit: 'fixture', thinkingLevels: ['high'], models: [] };
  ...
  else if (path.endsWith('/compare')) body = comparison;
...
for (const capture of [
  ...
  await page.locator('.run-trend article').first().waitFor();

  const compareButton = page.getByRole('button', { name: 'Compare with this run' }).first();
  const hasCompare = await compareButton.count() > 0;
  if (hasCompare && label === 'after') {
    await compareButton.scrollIntoViewIfNeeded();
    await compareButton.click();
    const panel = page.locator('[aria-label="Run comparison"]');
    await panel.waitFor();
  ...
  const fileName = `qs-83-run-compare-${label}-${capture.name}.png`;
  await page.screenshot({ path: join(output, fileName), fullPage: false });
  evidence.push({ ...capture, fileName, hasCompareEntryPoint: hasCompare, comparisonShown: hasCompare && label === 'after' });
  await page.close();
}

await browser.close();
await writeFile(join(output, `qs-83-run-compare-${label}-evidence.json`), `${JSON.stringify({ capturedAt: '2026-08-27T18:00:00Z', label, baseUrl, evidence }, null, 2)}\n`);
console.log(JSON.stringify({ output, label, evidence }, null, 2));
