...
- Production bundle budgets are enforced at 350 KB warning / 484 KB error initially and 10/12.5 KB per component stylesheet. The attack matrix is a deferred 17 KB lazy chunk and is not paid on the editor's first-content path. The error ceilings moved from 480/12 KB in QS-78 to fit the last-active-project memory and API-offline notice bar (measured production build: 481.42 KB initial, 12.22 KB `app.css`); `editor.css` (11.06 KB) and `review-panel.css` (11.99 KB) were already within a few hundred bytes of the old 12 KB ceiling before this change, so the per-component budget had no real headroom left for design-token-based styling.
