...
- Production bundle budgets are enforced at 350 KB warning / 490 KB error initially and 10/12 KB per component stylesheet. The attack matrix is a deferred 17 KB lazy chunk and is not paid on the editor's first-content path. QS-84 raised the initial error budget from 480 KB to 490 KB: highlighted finding spans and the finding ignore list moved the measured initial bundle from 478.30 KB to 486.16 KB (+7.86 KB), which is 6.16 KB above the previous 480 KB ceiling. The ignore-list styles live in a second `review-panel` stylesheet because `review-panel.css` is already at 11.99 KB against the 12 KB per-stylesheet error budget.
