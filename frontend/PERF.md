...
median of 7 samples, via `scripts/perf/switch-conditional-perf.mjs` (rerun
2026-08-27 on this host; absolute numbers move with host load, the 304/0-byte
outcome does not):

| Repository switch, agent-taskboard | Critical path | Transferred | Status |
| --- | ---: | ---: | --- |
| Cold — first switch, nothing retained | 981.33 ms | 33.89 MiB | 200 |
| Warm re-switch, before P-1 (unconditional) | 485.19 ms | 33.89 MiB | 200 |
| Warm re-switch, after P-1 (conditional) | **27.69 ms** | **0 B** | 304 |

The warm re-switch is 17.5× faster and transfers 35,534,898 fewer bytes. It now
fits the `< 500 ms` click-to-usable contract with margin on the 5,116-file
target, where the dossier recorded 7 of 7 samples over budget before this
slice. The cold switch is unchanged by this slice — it has no validator to
send, and the tree is still 33.89 MiB; that is what slices P-2 and P-3
address.
...
switches measured 8.0/13.7/30.4 ms to a visible transition and 217.7/43.5/87.8
