...
public sealed class ProjectDashboardService
{
        ...
        RepositoryHierarchySnapshot snapshot)
    {
        ...
        var key = CacheKey(root, snapshot);
    ...
    /// <summary>
    /// Seeds a projection the prewarmer already built, so the first request for a repository is a
    /// cache hit rather than a rebuild of work that has just been done.
    /// </summary>
    public void Seed(
        string repositoryPath,
        RepositoryHierarchySnapshot snapshot,
        ProjectDashboardResponse dashboard)
    {
        ...
        Store(CacheKey(Path.GetFullPath(repositoryPath), snapshot), dashboard);
    }

    private static string CacheKey(string root, RepositoryHierarchySnapshot snapshot) =>
        root + "\0" + snapshot.GitState;
