...
`backend/QualityStudio.Api/Program.cs` registers `GuidelineStore` as `Singleton` (stateless,
...
- 1.0.0 (2026-08-27): Initial rule, grounded in the `Singleton`/`Transient` split for
  ...
  `ReviewExecutorFactory` in `backend/QualityStudio.Api/Program.cs` and `ReviewJobs.cs`.