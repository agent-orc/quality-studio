...
`src/AgentOrchestrator.CodeQuality` / `src/QualityStudio.Api` (.NET).
Examples with a repository-relative `source` are real excerpts; examples
without one are explicitly captioned target shapes or contrasts against the
source-backed code.

The seed was also cross-checked on 2026-08-27 against the actual
`agent-orc/agent-studio` codebase rather than inferred from generic framework
guidance:

| Rule area | Quality Studio evidence | Agent Studio evidence |
| --- | --- | --- |
| Angular component structure and change detection | `frontend/src/app/review-panel/review-panel.ts` | `frontend/src/app/components/studio-sidebar-header/studio-sidebar-header.component.ts` and `frontend/src/app/components/dialog/dialog.component.ts` use standalone components, signal inputs, external template/style files, and `OnPush`; `frontend/AGENTS.md` defines the folder-per-component contract. |
| Design tokens | `frontend/src/styles.css` and component CSS consumers | `frontend/src/styles/_tokens-semantic.scss` owns the `--studio-spacing-*` scale and semantic aliases; `frontend/AGENTS.md` requires token-backed spacing and scoped deterministic Stylelint enforcement. |
| Standard-component reuse | The repeated dialog and badge defects cited by QS-NG-005/006 | `frontend/src/app/components/dialog/`, `count-badge/`, and `tree-row/` are the shared primitives consumed by feature templates instead of local copies. |
| .NET API, DI, and async hygiene | `src/QualityStudio.Api/Program.cs` and `src/AgentOrchestrator.CodeQuality/ReviewRunner.cs` | `backend/Host/Program.cs` centralizes service lifetimes; `backend/Features/Conversation/ConversationEndpoints.cs` binds and forwards the request cancellation token and awaits with `ConfigureAwait(false)`. |
| .NET test isolation | `tests/AgentOrchestrator.CodeQuality.Tests/GuidelineImpactAnalyzerTests.cs` | `runner.Tests/GitWorkspaceTests.cs` and `task-server.Tests/TaskServerStoreTests.cs` use GUID-scoped temporary roots with recursive cleanup. |

The canonical `good.source` and `bad.source` values stay local to Quality
Studio so `RuleLibraryTests.Every_example_source_points_at_a_real_path_in_this_repository`
can verify them deterministically. The table records the second-codebase
grounding without making rule loading depend on a sibling checkout.
...
An off-by-default rule installed deliberately from the catalogue is preserved
by sync; it is removed only when the project explicitly sets `enabled: false`
or the library deprecates it.
