...
`good` example is a real excerpt with a repository-relative `source` path;
every `bad` example is either a real excerpt of an existing defect or an
explicit contrast against one.
