...
### Agent Studio cross-codebase grounding

The seed set was also checked against Agent Studio at commit
`f955142acb4472720ea39290c164019118f2707b` (2026-08-25), rather than inferred
from Quality Studio alone. Rule-file `source` values remain Quality Studio
repository-relative so local validation can prove that every cited source
exists. The corresponding Agent Studio evidence is recorded here:

| Rule area | Agent Studio evidence at the pinned commit |
| --- | --- |
| Component structure and change detection | `docs/quality/angular-components.md` requires standalone, one-component-per-folder components, `ChangeDetectionStrategy.OnPush`, signal-backed state, and stable identity tracking. Concrete signal/OnPush components include `frontend/src/app/components/count-badge/count-badge.component.ts`, `frontend/src/app/components/tree-row/tree-row.component.ts`, and `frontend/src/app/components/dialog/dialog.component.ts`. |
| Design tokens | `frontend/AGENTS.md` defines `--studio-spacing-1` through `--studio-spacing-7`, forbids raw `px` for spacing, and requires semantic `--studio-*` theme tokens. `frontend/src/styles/_tokens-semantic.scss` is the central scale. |
| Standard-component reuse | `docs/quality/angular-components.md` says to use the canonical component or mixin before adding a local primitive. `frontend/AGENTS.md` names `app-tree-row`, `app-section-header`, `app-count-badge`, and `app-list-row`; `frontend/src/app/components/` contains those shared implementations. |
| Async hygiene | `backend/Features/Runtime/RuntimeEventWriter.cs`, `backend/Features/Persistence/IJsonlAppender.cs`, and `backend/Features/Git/GitNetworkProcessRunner.cs` propagate cancellation tokens and use `ConfigureAwait(false)` across file, semaphore, and process I/O. |
| API, DI, and tests | `docs/quality/dotnet-backend.md` requires stable route contracts, bounded feature flows, and direct matrix tests for pure policy. `backend/Host/Program.cs` owns explicit DI registrations and lifetimes, while `backend.Tests/Architecture/FeatureFolderBoundaryTests.cs` and the feature tests keep architecture and failure paths executable. |

This evidence is a grounding snapshot, not an external runtime dependency: the
Quality Studio rule library remains self-contained, English, and file-first in
this repository.
...
`catalogues/` already are). Installing an opt-in rule
...
### Automatic defaults and optional materialization

Every `defaultOn: true` rule applies to every project without a manual
install or sync step. `InputResolver` reads `.quality/rules.config.json`,
selects active shipped rules, and injects them as named project context
directly from the embedded `rules/` library. This read path does not dirty
the reviewed repository. Automatic prompt context contains each rule's title,
statement, rationale, effective severity, and autofixability; the canonical
file and optional materialized copy retain the fuller good/bad examples. This
keeps the complete core inside the established input budget. The core is 9 of
the 18 seeded rules — a rule earns a place in
...
materialization tool: it computes the same effective set and installs,
updates, or removes `.quality/inputs/<id>.md` copies. It is idempotent — call
it again and unchanged rules report `"unchanged"`. Reach it through
`POST /api/guidelines/sync-defaults` (and the paired
`/api/repos/{repoId}/...` route). Materialization is useful when a team wants
to inspect the generated Markdown in its repository, but reviews do not depend
on it.

When invoked, sync **overwrites** the installed file's content, priority,
...
change. `scripts/rule-checks/design-tokens-check.mjs` scans component CSS for
raw hex colors and raw `px` literals that duplicate values already declared
by central tokens (QS-NG-003), and uncovered literals repeated across
stylesheets (QS-NG-004). It ignores token declarations, comments, and the
accepted `1px` hairline exception, then writes a SARIF report citing those
rule ids. Register it as a deterministic sensor per project via
...
`severity` override becomes the effective default rendered in the named
prompt context and changes that rule's input priority. The review agent can
still deviate when concrete impact justifies it, so the override is guidance
rather than a hard cap or floor. `reason` is not schema-enforced but is expected in review,
