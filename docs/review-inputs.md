...
guideline. The rule library's `defaultOn` rules resolve automatically into every
project's named review context, subject to the repository's optional
`.quality/rules.config.json` overrides. No generated file or onboarding call is
required; `POST /api/guidelines/sync-defaults` is only the optional way to
materialize the effective rules under `.quality/inputs`. See the rule-library
