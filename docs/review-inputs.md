...
guideline. The rule library's `defaultOn` rules join the project inputs in
memory on every resolution, subject to the project's optional, versioned
`.quality/rules.config.json` overrides. No install or central settings write is
required. `POST /api/guidelines/sync-defaults` remains an optional way to
materialize editable copies under `.quality/inputs`; see the rule-library doc
for the full default-on/override contract.
