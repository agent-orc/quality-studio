...
guideline. The resolver supplies the rule library's `defaultOn` rules directly
for every project, without a setup call or generated project file, subject to the
project's optional `.quality/rules.config.json` overrides. The
`POST /api/guidelines/sync-defaults` endpoint remains an optional way to
materialize editable Markdown copies. See the rule-library dossier for the full
default-on/override contract.
