...
A sweep that reaches its own natural end (not capped or cancelled) ends `done` only when every file succeeded. If at least one file failed, the run ends `partial` when some files still succeeded and `failed` when none did. `failedFiles` and `completedFiles` are counted independently of this decision — a `failed` run can still report `completedFiles: 1` because that count includes every file the sweep finished attempting (succeeded or not), not just the ones that produced a usable review.
