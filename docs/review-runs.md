...
rendered prompts far above the Windows 32,767-character process command-line limit; the
reviewer therefore could fail before a Claude process attached. Stdin removes that launch
limit and keeps repository content out of process listings. Reviewer launch, attached PID,
CLI path, and log directory are emitted through the API host logger; raw per-stream runner
logs are stored under `ReviewJobs:ReviewerLogDirectory`.
