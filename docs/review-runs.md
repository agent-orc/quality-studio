...
Run orchestration is durable under `<project-data-root>/runs/<runId>/`:
...
`<project-data-root>/reports/runs/<runId>.json`. This canonical, strict-schema snapshot is
...
`runs/` remains outside Git because it is disposable orchestration working
