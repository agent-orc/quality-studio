...
local durable history. A capped run publishes revision 1; resuming and
...
The entire `.quality/` namespace is external runtime data and is ignored by Git at
every checkout depth. Review sidecars are current local truth; canonical run reports
preserve terminal execution history until an operator explicitly exports them.
