...
## Finding-level Ignore list

The repository-owned `.quality/findings/suppressions.json` policy mutes exact
finding fingerprints without deleting or rewriting their review observations.
The API is the only writer. It validates that the fingerprint belongs to the
selected review, requires author and reason, supports an optional expiry, and
uses the document revision for optimistic concurrency. Active finding views and
the effective grade exclude a current ignore match. The Review panel's Ignore
list still exposes the observation, policy reason, expiry, and Restore action.
After expiry or Restore, the finding returns to the active queue. Lifecycle
assessment and remediation state remain independent from ignore policy.
