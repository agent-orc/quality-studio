...
Finding lifecycle state is project-owned at
`<data-root>/<project-id>/.quality/findings/state.json`, outside the analysed
checkout. Records are keyed by fingerprint and contain `open`, `accepted`,
`waived`, `false-positive`, or `resolved`, plus author, reason, timestamp, and
optional expiry. New findings start open. A finding absent from the replacement
review is retained as resolved. A resolved finding that reappears becomes open.
Expired accepted, waived, or false-positive state also becomes open.
