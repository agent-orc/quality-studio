...
The Studio writes `coverage/coverage.json` below the external project data directory.
The snapshot records the measured commit and timestamp. A successful scan with no matching reports writes an empty snapshot, so every API and UI surface returns `unknown`, not an assumed `0%`.
