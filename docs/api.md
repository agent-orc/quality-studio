...
Runtime data is separate from this source root. `QualityStudio:DataRoot` selects its
base and defaults to `%LOCALAPPDATA%/QualityStudio/projects` on Windows; each
registration uses its ID as a child directory. The data root must be outside the
checkout. See [the operations contract](operations/data-root.md) for migration,
layout, exports, and recovery.
...
# 201 and appends attacks/coverage-ledger.jsonl below the project data root
...
# also writes boundaries/inventory.json below the project data root
...
# 201 and writes inputs/api-boundaries.md below the project data root
