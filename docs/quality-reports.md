...
Finding state comes from `findings/state.json` in the external project data root. Open and accepted
...
`reports/runs/<runId>.json` below the external project data root. The snapshot contains its immutable subject
