...
public sealed class ApiSmokeTests : IAsyncLifetime
{
    ...
    public async Task Tree_v2_returns_one_level_with_aggregate_facts_timing_and_conditional_cache()
    {
        ...
        var snapshotEtag = json.GetProperty("snapshotEtag").GetString();
        Assert.False(string.IsNullOrWhiteSpace(snapshotEtag));
        ...
        var selectedSnapshot = Uri.EscapeDataString(snapshotEtag!);
        using var childrenResponse = await client.GetAsync(
            $"/api/tree/v2?parentId={projectId}&snapshot={selectedSnapshot}&limit=100",
    ...
    public async Task Tree_v2_search_finds_unloaded_files_without_returning_descendants()
    {
        ...
        // The hit states what is below it without shipping any of it.
        Assert.True(file.GetProperty("hasChildren").GetBoolean());
        Assert.Equal(1, file.GetProperty("childCount").GetInt32());
    ...
    public async Task Boundary_sensor_scan_persists_repository_owned_inventory()
    {
        ...
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
        var path = BoundaryInventorySensor.InventoryPathFor(repositoryRoot);
        Assert.True(File.Exists(path));
    ...
    public async Task Finding_state_action_projects_state_and_rejects_a_conflicting_write()
    {
        ...
        var metadataPath = ReviewMetaPath.ForFile(repositoryRoot, "Sample.cs", "code");
    ...
    public async Task Finding_ignore_list_persists_exact_fingerprint_and_restores_observation()
    {
        ...
        var metadata = new ReviewMetaDocument
        {
            Unit = new ReviewUnit("qs-v1/generic/file/" + new string('e', 64), ReviewAdapter.Generic,
                ReviewLevel.File, "Sample.cs", "Sample.cs"),
            ...
            Summary = "One finding.",
            Aspects = [new ReviewAspect("correctness", "Correctness", grade)],
            Findings = [new ReviewFinding(findingId, "correctness", FindingSeverity.High, "Persistent ignored finding",
                "A finding used by the ignore-list API test.", "Review it.", [new FindingLocation("Sample.cs")],
                fingerprint, "correctness.ignore-test")],
        };
        await File.WriteAllTextAsync(
            metadataPath, ReviewMetaJson.Serialize(metadata), TestContext.Current.CancellationToken);
        ...
        try
        {
            ...
            var suppressedGrade = file.GetProperty("metaDocuments")[0].GetProperty("grade").GetProperty("score").GetInt32();
