...
public sealed class ReviewRunArchiveStoreTests
{
    ...
    public void History_reader_pages_filters_selects_attempts_and_keeps_corrupt_rows_visible()
    {
        ...
        var usage = new[]
        {
            new ReviewUsageEntry("provider-archived", CreatedAt, "gpt-5", "codex",
                new TokenUsage(10, 2, null, null, 30), "security", "project", ".",
                "review-newer", 2),
            new ReviewUsageEntry("provider-legacy", CreatedAt.AddDays(2), "gpt-5.1", "codex",
                new TokenUsage(25, 5, 3, null, 40), "code", "file", "src/Legacy.cs",
                "review-legacy", 2),
        };
        var withLegacy = ReviewRunHistoryReader.List(
            fixture.Store, "default", null, 20, "code", "src/Legacy.cs", "legacy-usage-only", usage);
        var legacy = Assert.Single(withLegacy.Runs, row => row.RunId == "review-legacy");
        Assert.Equal("legacy-usage-only", legacy.Provenance);
        Assert.Equal("gpt-5.1", legacy.Model);
        Assert.Equal(25, legacy.Spend?.Tokens.InputTokens);
        Assert.Null(legacy.CreatedAt);
        Assert.DoesNotContain(withLegacy.Runs, row => row.RunId == "review-newer" &&
            row.Provenance == "legacy-usage-only");
    }
