...
public sealed class ReviewRunArchiveStoreTests
{
    ...
    [Fact]
    public void Repository_paths_in_archive_records_reject_traversal_and_absolute_paths()
    {
        using var fixture = new ArchiveFixture();
        var run = CreateRun();

        Assert.Throws<ArgumentException>(() => fixture.Store.CreateRun(run with
        {
            Subject = run.Subject with { Path = "../outside" },
        }));
        Assert.Throws<ArgumentException>(() => fixture.Store.CreateRun(run with
        {
            Targets = [run.Targets[0] with { Path = Path.GetFullPath("outside.cs") }],
        }));

        fixture.Store.CreateRun(run);
        Assert.Throws<ArgumentException>(() => fixture.Store.AppendOperation(run.CreatedAt,
            CreateOperation(run.RunId) with { ResultSidecar = "../../outside.json" }));
        Assert.Throws<ArgumentException>(() => fixture.Store.AppendFinding(run.CreatedAt,
            CreateFinding(run.RunId, "operation-0001") with
            {
                Locations = [new FindingLocation("../outside.cs")],
            }));
    }
