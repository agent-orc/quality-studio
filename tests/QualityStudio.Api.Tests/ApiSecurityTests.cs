...
public sealed class ApiSecurityTests : IAsyncLifetime
{
    ...
    public async Task Repository_administration_requires_registrar_and_analyzers_use_host_owned_profiles()
    {
        ...
        var listed = await alice.GetFromJsonAsync<JsonElement>(
            "/api/repos", TestContext.Current.CancellationToken);
        var defaults = Assert.Single(listed.GetProperty("repositories").EnumerateArray());
        var defaultSarif = Assert.Single(defaults.GetProperty("sensors").EnumerateArray(),
            sensor => sensor.GetProperty("id").GetString() == "sarif");
        Assert.False(defaultSarif.GetProperty("enabled").GetBoolean());
