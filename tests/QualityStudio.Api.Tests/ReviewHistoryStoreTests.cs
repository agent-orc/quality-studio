...
public sealed class ReviewHistoryStoreTests
{
    ...
    public void TerminalHistory_IsCreateOnlyIdempotentPortableAndIndependentOfActiveState()
    {
        ...
        try
        {
            ...
            Assert.Null(first.Run.ExecutedRoute.Cli);
            Assert.Null(first.Run.ExecutedRoute.Model);
            Assert.Null(first.Run.ExecutedRoute.ThinkingLevel);
    ...
    public void OperationEvidenceCapturesOnlyTypedReferencesHashesAndRoutes()
    {
        ...
        try
        {
            ...
            var (manifest, status, progress) = TerminalRun(root.FullName, "operation-route");
            var history = new ReviewHistoryStore(root.FullName).Commit(manifest, status, progress, [captured]);
            Assert.Equal("codex", history.Run.ExecutedRoute.Cli);
            Assert.Equal("gpt-5.1-codex", history.Run.ExecutedRoute.Model);
            Assert.Equal("high", history.Run.ExecutedRoute.ThinkingLevel);
            Assert.Null(history.Run.RequestedRoute.Model);
            Assert.Equal("high", history.Run.RequestedRoute.ThinkingLevel);
        }
        finally
        {
            root.Delete(true);
