...
public sealed class ReviewRunDiffTests
{
    ...
    [Fact]
    public async Task Diff_uses_the_latest_same_attempt_retry_for_each_path()
    {
        var completed = Operation("after", "operation-completed", "a", "src/A.cs", 90, "policy-1") with
        {
            FinishedAt = Time.AddSeconds(2),
        };
        var after = Detail(
            "after", "gpt-5", "done", true,
            [Target("a", "src/A.cs", "hash-a")],
            [Operation("after", "operation-cancelled", "a", "src/A.cs", 40, "policy-1") with
                { State = "cancelled", FinishedAt = Time.AddSeconds(1) }, completed],
            [],
            new TokenUsage(10, 2, null, null, 100), null);
        var before = after with
        {
            Run = after.Run with { RunId = "before" },
            Operations = [completed with { RunId = "before", OperationId = "operation-before" }],
        };

        var diff = await ReviewRunDiffService.CompareAsync(Path.GetTempPath(), before, after,
            cancellationToken: TestContext.Current.CancellationToken);

        var grade = Assert.Single(diff.Grades);
        Assert.Equal(90, grade.Before?.Score);
        Assert.Equal(90, grade.After?.Score);
        Assert.Equal(0, grade.ScoreChange);
    }
