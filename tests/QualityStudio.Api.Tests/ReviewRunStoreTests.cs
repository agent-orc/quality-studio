...
public sealed class ReviewRunStoreTests
{
    ...
    public async Task Snapshot_preflight_runs_configured_sensor_once_and_persists_the_exact_estimate_evidence()
    {
        ...
        try
        {
            ...
            var promptRunner = new ReviewRunner(sensorRegistry: new SensorRegistry([sensor]));
            var measuredCharacters = 0L;
            foreach (var request in executor.Requests)
            {
                measuredCharacters += (await promptRunner.MeasurePromptAsync(request, cancellationToken)).Characters;
            }
            Assert.Equal(
                completed.GetProperty("estimate").GetProperty("promptCharacters").GetInt64(),
                measuredCharacters);
    ...
    [Fact]
    public async Task Tool_version_mutation_invalidates_preflight_before_the_next_model_operation()
    {
        var cancellationToken = TestContext.Current.CancellationToken;
        var fixture = await DurableRunFixture.CreateAsync(cancellationToken);
        var sensor = new CountingPreflightSensor(available: true);
        var executor = new CapturingExecutorFactory(operation =>
        {
            if (operation == 1) sensor.ToolVersion = "2.0.0";
        });
        try
        {
            await using var application = fixture.CreateApplication(
            ...
            Assert.Equal(2, sensor.RunCount);
            var hashes = executor.Requests.Select(request => Assert.Single(request.PreflightEvidence!).ResultHash).ToArray();
    ...
    private sealed class CapturingExecutorFactory(Action<int>? operationStarted = null) : IReviewExecutorFactory
    {
        private readonly List<ReviewRequest> requests = [];
        private readonly Action<int>? onOperationStarted = operationStarted;
        ...
        public IReviewExecutor Create(string cliType, string? model, string? thinkingLevel, Action<string, CliRunEvent> eventObserver,
            Action<ReviewUsageEntry> usageRecorded) => new CapturingExecutor(this);

        private sealed class CapturingExecutor(CapturingExecutorFactory owner) : IReviewExecutor
        {
            public Task<ReviewExecutionResult> ReviewIfNeededAsync(
                ReviewRequest request,
                bool force,
                CancellationToken cancellationToken)
            {
                ...
                owner.onOperationStarted?.Invoke(Interlocked.Increment(ref owner.operations));
    ...
    private sealed class CountingPreflightSensor(bool available) : ISecurityEvidenceSensor
    {
        ...
        public Task<SensorAvailability> ProbeAvailabilityAsync(CancellationToken cancellationToken = default) =>
            Task.FromResult(new SensorAvailability(
                true,
                ToolVersions: new Dictionary<string, string> { [Id] = ToolVersion }));
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            return Task.FromResult(new SensorScanResult(
                ...
                new SensorProvenance(
                    ...
                    new Dictionary<string, string> { [Id] = ToolVersion })));
        }
    }
