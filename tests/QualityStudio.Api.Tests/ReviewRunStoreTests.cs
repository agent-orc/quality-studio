...
public sealed class ReviewRunStoreTests
{
    ...
    public async Task Angular_compiler_runs_before_model_projects_affected_finding_and_can_be_disabled()
    {
        ...
        var typeScriptDirectory = Path.Combine(
            fixture.RepositoryRoot, "frontend", "node_modules", "typescript");
        const string subjectPath = "frontend/src/app/editor/editor.html";
        Directory.CreateDirectory(Path.Combine(compilerDirectory, "bundles", "src", "bin"));
        Directory.CreateDirectory(typeScriptDirectory);
        Directory.CreateDirectory(Path.Combine(
            fixture.RepositoryRoot, "frontend", "src", "app", "editor"));
        await File.WriteAllTextAsync(Path.Combine(
            fixture.RepositoryRoot, "frontend", "tsconfig.app.json"), "{}", cancellationToken);
        await File.WriteAllTextAsync(Path.Combine(
            fixture.RepositoryRoot, "frontend", "angular.json"),
        ...
        await File.WriteAllTextAsync(Path.Combine(typeScriptDirectory, "package.json"),
            "{\"version\":\"5.9.3\"}", cancellationToken);
        await File.WriteAllTextAsync(Path.Combine(
            compilerDirectory, "bundles", "src", "bin", "ngc.js"), "#!/usr/bin/env node",
            cancellationToken);
        await File.WriteAllTextAsync(Path.Combine(
            fixture.RepositoryRoot, subjectPath.Replace('/', Path.DirectorySeparatorChar)),
            "<input>", cancellationToken);
        var sensor = new AngularCompilerSensor(
            new AngularOrderingRunner(events), fixture.RepositoryRoot);
        ...
        try
        {
            ...
            var angularDescriptor = Assert.Single(
                descriptors.GetProperty("sensors").EnumerateArray(),
            ...
            await WaitForStateAsync(
                client, disabled.GetProperty("id").GetString()!, "done", cancellationToken);
            ...
            CancellationToken cancellationToken = default)
        {
            if (arguments.Contains("-p", StringComparer.Ordinal))
            {
                ...
                return Task.FromResult(new SensorCommandResult(0, """
                    src/app/editor/editor.html:171:61 - warning NG8102: The nullish fallback is unreachable.

                    171 <input [value]="draft ?? ''">
                                                                ~~~~~~~~~~~
                    """, string.Empty));
            }
