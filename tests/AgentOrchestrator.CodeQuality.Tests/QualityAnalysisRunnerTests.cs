...
{
    [Fact]
    public void Default_runner_exposes_the_supported_analysis_names()
    {
        var names = new QualityAnalysisRunner().AvailableAnalyses;
    ...
    [Fact]
    public void Core_assembly_has_no_AspNetCore_or_Agent_Studio_transport_type()
    {
        var assembly = typeof(QualityAnalysisRunner).Assembly;

        Assert.DoesNotContain(assembly.GetReferencedAssemblies(), reference =>
            reference.Name?.StartsWith("Microsoft.AspNetCore", StringComparison.Ordinal) == true);
        Assert.Null(assembly.GetType("QualityStudio.Api.AgentStudioTaskClient"));
    }
