...
public sealed class PreflightTests
{
    ...
    [Fact]
    public async Task Tool_version_change_invalidates_the_configuration_cache_key()
    {
        var sensor = new CountingSensor("fixture", available: true, []);
        var collector = new PreflightCollector(new SensorRegistry([sensor]));
        var configuration = new[] { new ReviewSensorConfiguration("fixture") };

        var first = await collector.ConfigurationSetHashAsync(
            configuration, TestContext.Current.CancellationToken);
        sensor.ToolVersion = "2.0.0";
        var changed = await collector.ConfigurationSetHashAsync(
            configuration, TestContext.Current.CancellationToken);
