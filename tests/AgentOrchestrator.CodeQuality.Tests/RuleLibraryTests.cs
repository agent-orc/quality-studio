...
public sealed class RuleLibraryTests
{
    ...
    [Fact]
    public void SyncDefaultRules_preserves_a_manually_installed_opt_in_rule()
    {
        var root = Path.Combine(Path.GetTempPath(), "rule-library-sync-opt-in-tests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        try
        {
            var store = new GuidelineStore();
            var optInRule = RuleLibrary.Rules.First(rule => !rule.DefaultOn && rule.Status == "active");
            store.Install(root, optInRule.Id);

            var results = store.SyncDefaultRules(root);

            Assert.DoesNotContain(results, result => result.RuleId == optInRule.Id && result.Action == "removed");
            Assert.Contains(store.List(root), guideline => guideline.Id == optInRule.Id);
        }
        finally
        {
            Directory.Delete(root, true);
