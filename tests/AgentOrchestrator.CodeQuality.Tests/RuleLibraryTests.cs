...
public sealed class RuleLibraryTests
{
    ...
    public void Every_rule_file_on_disk_validates_against_the_rule_schema(string path)
    {
        ...
        Assert.Equal(document.RootElement.GetProperty("version").GetString(),
            document.RootElement.GetProperty("changelog")[0].GetProperty("version").GetString());
    }
    ...
    public void The_default_on_core_is_exactly_the_set_documented_in_the_dossier()
    {
        ...
        // docs/concepts/rule-library.md#default-on-sync. Changing this set is a deliberate act:
    ...
    public void RuleConfig_missing_file_yields_empty_overrides()
    {
        ...
        try
        {
            ...
            Assert.True(config.IsEnabled(RuleLibrary.Get("QS-NG-003")));
            Assert.False(config.IsEnabled(RuleLibrary.Get("QS-NG-002")));
        }
        finally
        {
            Directory.Delete(root, true);
    ...
    [Fact]
    public void InputResolver_applies_defaults_without_materializing_files_and_surfaces_project_overrides()
    {
        var root = Path.Combine(Path.GetTempPath(), "rule-library-resolver-tests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(Path.Combine(root, ".quality"));
        File.WriteAllText(Path.Combine(root, ".quality", "rules.config.json"), """
        {
          "$schema": "https://quality.studio/schemas/rule-config.v1.schema.json",
          "schemaVersion": 1,
          "overrides": {
            "QS-NG-003": { "enabled": false, "reason": "covered by stylelint" },
            "QS-NG-004": { "severity": "high", "reason": "token drift is release-critical here" },
            "QS-NG-008": { "enabled": true, "reason": "all lists in this project reorder" }
          }
        }
        """);
        try
        {
            ...
            var ids = resolved.Inputs.Select(input => input.Id).ToHashSet(StringComparer.Ordinal);

            Assert.DoesNotContain("QS-NG-003", ids);
            Assert.Contains("QS-NG-004", ids);
            Assert.Contains("QS-NG-008", ids);
            ...
            Assert.Contains("library default: medium", adjusted.Content, StringComparison.Ordinal);
            Assert.Contains("token drift is release-critical here", adjusted.Content, StringComparison.Ordinal);
    ...
    [Fact]
    public void Complete_default_on_core_fits_the_standard_prompt_budget()
    {
        var root = Path.Combine(Path.GetTempPath(), "rule-library-budget-tests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(root);
        try
        {
            var resolved = new InputResolver().Resolve(root, "code", ReviewLevel.File);
            var expected = RuleLibrary.Rules.Count(rule => rule.DefaultOn && rule.Status == "active" &&
                rule.Kinds.Contains("code") && rule.Levels.Contains("file"));

            Assert.Equal(expected, resolved.Inputs.Count);
            Assert.All(resolved.Inputs, input =>
            {
                Assert.False(input.Truncated, $"{input.Id} was truncated by the standard prompt budget.");
                Assert.NotEmpty(input.IncludedContent);
            });
            Assert.True(resolved.Complete);
        }
        finally
        {
            Directory.Delete(root, true);
