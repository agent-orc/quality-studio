...
public sealed class RuleLibraryTests
{
    ...
    [Fact]
    public void Every_loaded_rule_exposes_current_change_history()
    {
        Assert.All(RuleLibrary.Rules, rule =>
        {
            Assert.NotEmpty(rule.Changelog);
            Assert.Equal(rule.Version, rule.Changelog[0].Version);
        });
    }
    ...
    public void The_default_on_core_is_exactly_the_set_documented_in_the_dossier()
    {
        ...
        // docs/concepts/rule-library.md#automatic-defaults-and-optional-materialization. Changing this set is a deliberate act:
    ...
    [Fact]
    public void InputResolver_applies_defaults_and_project_overrides_without_materializing_files()
    {
        ...
        File.WriteAllText(Path.Combine(root, ".quality", "rules.config.json"), """
        {
          ...
          "schemaVersion": 1,
          "overrides": {
            "QS-NG-003": { "severity": "critical", "reason": "release-specific visual contract" },
            "QS-NG-004": { "enabled": false, "reason": "external stylelint rule" },
            "QS-NG-001": { "enabled": true, "reason": "project component convention" }
          }
        }
        """);
        try
        {
            ...
            Assert.Contains(resolved.Inputs, input => input.Id == "QS-NG-001");
            Assert.DoesNotContain(resolved.Inputs, input => input.Id == "QS-NG-004");
            var adjusted = Assert.Single(resolved.Inputs, input => input.Id == "QS-NG-003");
            Assert.Contains("Severity: critical (project override; library default: medium)", adjusted.Content,
                StringComparison.Ordinal);
            Assert.Contains("release-specific visual contract", adjusted.Content, StringComparison.Ordinal);
            Assert.False(File.Exists(Path.Combine(root, ".quality", "inputs", "QS-NG-003.md")));
        }
        finally
        {
            Directory.Delete(root, true);
    ...
    [Fact]
    public void RuleConfig_rejects_unknown_rules_instead_of_silently_ignoring_them()
    {
        ...
        try
        {
            var error = Assert.Throws<JsonException>(() => RuleConfig.Load(root));
            Assert.Contains("unknown rule 'QS-NG-999'", error.Message, StringComparison.Ordinal);
        }
        finally
        {
            Directory.Delete(root, true);
