...
test('launcher bootstraps a clean checkout, starts both services, and can restart cleanly', async t => {
  ...
  await writeFile(npmStub, npmInstallStub());
  ...
  await assertPortsReleased([firstApiPort, firstWebPort]);
...
test('launcher fails if API never becomes ready', async t => {
  ...
  const [apiPort, webPort] = await freePorts(2);
  await writeFile(apiScript, `console.error('api-startup-diagnostic'); process.exit(1);`);
  await writeFile(webScript, serviceScript('web-ready'));
  ...
  assert.match(`${result.stdout}\n${result.stderr}`, /Captured api output:[\s\S]*api-startup-diagnostic/);
});

test('launcher fails if frontend exits before ready', async t => {
...
test('embedded shell loads in an iframe and shows the live connection badge', async t => {
