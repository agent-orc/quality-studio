...
test('release canary retains every sample before enforcing its classified result', async () => {
  ...
  // The external-live lane fails without its opt-in by design, so the only workflow that
  // selects it has to supply that opt-in here - otherwise the canary could report green
  // for a live check that never ran.
  const liveStep = workflow.match(/      - name: Run explicitly approved live review check[\s\S]*?(?=\n      - name:)/)?.[0] ?? '';
  assert.match(liveStep, /--filter "Category=ExternalLive"/);
  assert.match(liveStep, /QUALITY_RUN_LIVE_REVIEW: "1"/);
