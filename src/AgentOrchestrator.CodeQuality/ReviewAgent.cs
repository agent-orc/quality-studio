        ...
        CancellationToken cancellationToken = default)
    {
            ...
            }, cancellationToken))
            {
                if (runEvent is CliRunEvent.RunStarted started)
                {
                    ...
                    _logger.LogInformation(new EventId(1601, "ReviewerCliAttached"),
                        "Reviewer operation {ReviewerOperationId} attached to {ReviewerCliType} PID {ReviewerProcessId}; CLI path: {ReviewerCliPath}; logs: {ReviewerLogDirectory}",
                        runId, _cliType, started.ProcessId, cliPath, logDirectory ?? "runner-default");
                }
        ...
        if (ended is null || ended.Outcome != RunOutcome.Completed || terminalFailure is not null)
        {
            ...
            throw new ReviewAgentRunException(
                ...
                completed.Usage,
                completed.Model,
                new InvalidOperationException($"Reviewer CLI {outcome}: {Tail(detail)}"),
                authenticationFailed ? "reviewer_authentication_required" : "reviewer_run_failed",
                retryable: !authenticationFailed);
        }
