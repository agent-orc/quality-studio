...
public sealed record PreflightToolIdentity(
    string Hash,
    IReadOnlyDictionary<string, SensorAvailability> Sensors);
...
public sealed class PreflightCollector(SensorRegistry registry)
{
    public async Task<PreflightSnapshot> CollectAsync(
        ...
        PreflightToolIdentity? toolIdentity = null)
    {
        ...
        toolIdentity ??= await ProbeToolsAsync(normalizedConfigurations, cancellationToken).ConfigureAwait(false);
        ...
        {
            results.Add(await CollectOneAsync(
                    repositoryRoot, subject, configuration, toolIdentity.Sensors, cancellationToken)
                .ConfigureAwait(false));
        }
        var tasks = normalizedConfigurations
            .Where(configuration => !IsPrerequisite(configuration.Id))
            .Select(configuration => CollectOneAsync(
                repositoryRoot, subject, configuration, toolIdentity.Sensors, cancellationToken));
        ...
        return new PreflightSnapshot(
            ...
            configurationHash,
            toolIdentity.Hash,
            HashSnapshot(subject, configurationHash, toolIdentity.Hash, ordered),
            ordered,
    ...
    public async Task<PreflightToolIdentity> ProbeToolsAsync(
        IReadOnlyList<ReviewSensorConfiguration> configurations,
        CancellationToken cancellationToken = default)
    {
        var states = new Dictionary<string, SensorAvailability>(StringComparer.OrdinalIgnoreCase);
                     ...
                     .OrderBy(item => item.Id, StringComparer.Ordinal))
        {
            try
            {
                states[configuration.Id] = await registry.Get(configuration.Id)
            ...
            {
                states[configuration.Id] = new SensorAvailability(
                    false, $"Tool probe failed: {exception.Message}");
            }
        }

        var canonical = new JsonArray(states
            .OrderBy(pair => pair.Key, StringComparer.Ordinal)
            .Select(pair => (JsonNode)new JsonObject
            {
                ["id"] = pair.Key,
                ["available"] = pair.Value.Available,
                ["unavailableReason"] = pair.Value.UnavailableReason,
                ["toolVersions"] = DictionaryJson(pair.Value.ToolVersions),
            }).ToArray()).ToJsonString();
        return new PreflightToolIdentity(Hash(canonical), states);
    }
    ...
    private async Task<PreflightResult> CollectOneAsync(
        ...
        IReadOnlyDictionary<string, SensorAvailability> toolStates,
        CancellationToken cancellationToken)
    {
        ...
        if (!toolStates.TryGetValue(configuration.Id, out var availability) || !availability.Available)
        {
            return Failed(
                subject,
                configuration,
                sensor.Version,
                availability?.UnavailableReason ?? "Tool availability was not probed.",
