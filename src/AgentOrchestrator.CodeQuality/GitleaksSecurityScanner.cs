...
public class GitleaksSecurityScanner : IReviewSensor
{
    ...
    public virtual async Task<SecurityScanResult> ScanAsync(SecurityScanRequest request, CancellationToken cancellationToken = default)
    {
        ...
        var configPath = ResolveOptionalPath(root, request.DataRoot, request.ConfigPath, ".quality/security/gitleaks.toml");
        var baselinePath = ResolveOptionalPath(root, request.DataRoot, request.BaselinePath, ".quality/security/gitleaks.baseline.json");
        ...
        {
            await PersistFindingsAsync(root, QualityDataPaths.Root(root, request.DataRoot), binaryPath, request, grouped, configPath, baselinePath, cancellationToken)
                .ConfigureAwait(false);
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var fileGroup in groups)
        {
            ...
            var sourceDirectory = Path.GetRelativePath(root, Path.GetDirectoryName(absolutePath)!);
            var metaPath = Path.Combine(dataRoot, sourceDirectory == "." ? string.Empty : sourceDirectory,
    ...
    private static string? ResolveOptionalPath(string root, string? dataRoot, string? supplied, string relativeDefault)
    {
        var relative = string.IsNullOrWhiteSpace(supplied) ? relativeDefault : supplied;
        var candidate = Path.IsPathRooted(relative)
            ? relative
            : QualityDataPaths.ResolveMutablePath(root, dataRoot, relative);
