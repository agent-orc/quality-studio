...
public sealed partial class GuidelineStore
{
    ...
    /// <summary>
    /// One installable entry per named rule in <see cref="RuleLibrary.Default"/>. Installing a rule
    /// is an explicit opt-in action, so the written guideline is always enabled, regardless of the
    /// rule's <c>defaultOn</c> flag; <c>defaultOn</c> only controls whether the rule already applies
    /// automatically through <see cref="InputResolver"/>'s built-in tier before it is installed.
    /// </summary>
    public static IReadOnlyList<GuidelineCatalogueEntry> Catalogue { get; } =
        RuleLibrary.Default.Rules.Select(rule => new GuidelineCatalogueEntry(
            rule.Id.ToLowerInvariant(), rule.Title, TechnologyLabel(rule.Technology), rule.Summary,
            new GuidelineDraft(rule.Id.ToLowerInvariant(), true, RulePriority(rule.Severity),
                rule.Kinds, rule.Levels, rule.Content))).ToArray();

    private static string TechnologyLabel(string technology) => technology switch
    {
    ...
    private static int RulePriority(string severity) => severity switch
    {
        "critical" => 900,
        "high" => 700,
        "medium" => 500,
