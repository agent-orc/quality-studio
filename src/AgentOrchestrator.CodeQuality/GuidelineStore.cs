...
public sealed partial class GuidelineStore
{
    ...
    /// design (see docs/concepts/rule-library.md#default-on-resolution).
    /// </summary>
    public IReadOnlyList<RuleSyncResult> SyncDefaultRules(string repositoryRoot)
    {
        ...
        {
            var enabled = rule.Status == "active" && config.IsEnabled(rule.Id, rule.DefaultOn);
            var draft = RuleLibrary.CreateGuidelineDraft(
                rule, config.EffectiveSeverity(rule), config.OverrideReason(rule.Id));
            var current = existing.GetValueOrDefault(rule.Id);
            if (!enabled)
            {
                // An off-by-default rule may have been installed deliberately from the catalogue.
                // Syncing the core must not erase that explicit project choice. Only an explicit
                // disable override (or library deprecation) authorizes removing a materialized rule.
                if (current is not null &&
                    (config.IsExplicitlyDisabled(rule.Id) || rule.Status == "deprecated"))
                {
