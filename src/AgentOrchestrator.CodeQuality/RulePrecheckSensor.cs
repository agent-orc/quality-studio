...
public sealed class RulePrecheckSensor : IDeterministicEvidenceSensor
{
    ...
    public RulePrecheckSensor(RuleLibrary? ruleLibrary = null) =>
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var configuration = QualityRuleConfiguration.Load(root, ruleLibrary);
        var tokenRule = EffectiveRule("QS-NG-002", configuration);
        var templateRule = EffectiveRule("QS-NG-004", configuration);
        ...
        foreach (var file in EnumerateSources(target))
        {
            ...
            if ((extension is ".css" or ".scss") && tokenRule is not null)
            {
                AnalyzeStyle(path, normalized, tokenRule, findings);
            }
            else if (file.EndsWith(".ts", StringComparison.OrdinalIgnoreCase) &&
                     !IsTestSource(file) && templateRule is not null)
            {
                AnalyzeComponentMetadata(path, normalized, templateRule, findings);
            }
        }
    ...
    private QualityRule? EffectiveRule(string id, QualityRuleConfiguration configuration)
    {
        var rule = ruleLibrary.List().Single(rule => rule.Id == id);
        return string.Equals(rule.Status, "active", StringComparison.OrdinalIgnoreCase) &&
               rule.DeterministicCheck is not null && configuration.IsEnabled(rule)
