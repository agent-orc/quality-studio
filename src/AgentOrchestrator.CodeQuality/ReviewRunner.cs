...
public sealed class ReviewRunner
{
    ...
    private async Task<PreparedPrompt> PreparePromptAsync(ReviewRequest request, CancellationToken cancellationToken)
    {
        ...
        var inputs = _inputResolver.Resolve(root, request.Kind, request.Level,
            request.GlobalInputsDirectory, request.InputBudgetCharacters, dataRoot);
        ...
        var unitId = request.UnitId ?? ResolveUnitId(root, dataRoot, relativePath, request.Level)
    ...
    private static string GetMetaPath(string root, string dataRoot, string firstFile, string kind, string relativePath, ReviewLevel level)
    {
        ...
        {
            ReviewLevel.Project => dataRoot,
            ReviewLevel.Module when File.Exists(Path.Combine(root, relativePath.Replace('/', Path.DirectorySeparatorChar)))
                => Path.GetDirectoryName(Path.Combine(dataRoot, relativePath.Replace('/', Path.DirectorySeparatorChar)))!,
            _ => Path.Combine(dataRoot, Path.GetRelativePath(root, Path.GetDirectoryName(firstFile)!)),
        };
    ...
    private static string? ResolveUnitId(string root, string dataRoot, string relativePath, ReviewLevel level) =>
        FlattenHierarchy(RepositoryHierarchyBuilder.Build(root, dataRoot))
