    ...
    private static void Validate(RuleDefinition rule, string source)
    {
        ...
        if (rule.Changelog is not { Count: > 0 } ||
            ...
            throw new JsonException(
                $"Rule resource '{source}' requires a changelog whose newest entry matches version '{rule.Version}'.");
        if (string.Equals(rule.Status, "deprecated", StringComparison.Ordinal) &&
            string.IsNullOrWhiteSpace(rule.SupersededBy))
            throw new JsonException($"Deprecated rule resource '{source}' must name its successor.");
