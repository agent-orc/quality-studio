...
public static class RuleLibrary
{
    ...
    public static GuidelineDraft EffectiveGuideline(
        RuleDefinition rule,
        RuleOverride? projectOverride = null,
        bool includeExamples = true)
    {
        ...
        return new GuidelineDraft(rule.Id, true, Priority(severity), rule.Kinds, rule.Levels,
            RenderContent(rule, severity, projectOverride?.Reason, includeExamples));
    }
        ...
        bool includeExamples = true)
    {
        ...
        if (!string.IsNullOrWhiteSpace(overrideReason))
            builder.Append(" Override reason: ").Append(overrideReason.Trim()).Append('.');
        builder.Append(" Rule version: ").Append(rule.Version).Append('.');
        if (includeExamples)
        {
