...
Named-rule and guideline headings contain stable rule ids. Set every finding's `ruleId` to the exact supplied id that caused it, and use the effective severity shown for a named rule. Use `built-in:performance` only for findings from the base review criteria. `ruleId` is required on every finding.
