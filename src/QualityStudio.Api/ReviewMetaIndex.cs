...
public sealed class ReviewMetaIndex : IDisposable
{
    ...
    public IReadOnlyList<JsonElement> Read(string root, string relativePath) =>
        Read(root, Path.Combine(Path.GetFullPath(root), ".quality"), relativePath);
    ...
    public string Find(string root, string relativePath, string kind) =>
        Find(root, Path.Combine(Path.GetFullPath(root), ".quality"), relativePath, kind);
    ...
    private RepositoryIndex Get(string root, string dataRoot) => repositories.GetOrAdd(
        Path.GetFullPath(root) + "\0" + Path.GetFullPath(dataRoot), _ => new(root, dataRoot));
