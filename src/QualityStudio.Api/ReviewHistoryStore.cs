...
public sealed class ReviewHistoryStore
{
    ...
    private static ReviewHistoryRoute ExecutedRoute(IReadOnlyList<ReviewRunOperationEvidence> evidence) => new(
        ...
        SingleObserved(evidence.Select(item => item.Executed?.Model)),
        SingleObserved(evidence.Select(item => item.Executed?.ThinkingLevel)));

    private static string? SingleObserved(IEnumerable<string?> values)
    {
        var observed = values.Where(value => !string.IsNullOrWhiteSpace(value))
