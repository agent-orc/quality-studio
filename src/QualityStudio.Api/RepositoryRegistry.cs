...
public sealed class RepositoryRegistry
{
    ...
    private RepositoryRegistration Validate(RepositoryRegistrationRequest request, string? existingId)
    {
        ...
        // Only a caller-supplied "command" is gated here: the host's own deterministic sensor defaults
        // are fixed and not attacker-influenced. A configured command such as a shell can read beyond its
        // working directory, inspect inherited credentials, and use the network (docs/operations/security/index.html, F-01).
        if (!legacyOptions.Security.AllowCommandBackedAnalyzers &&
            request.Sensors is { } explicitSensors &&
            explicitSensors.Any(sensor => sensor.Configuration is not null &&
