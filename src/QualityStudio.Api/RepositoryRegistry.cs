...
public sealed class RepositoryRegistry
{
        ...
        if (File.Exists(registryPath))
        {
            try
            {
                ...
                if (loaded is { Count: > 0 })
                {
                    ...
                    {
                        var reason = ValidatePersistedEntry(migrated[i]);
                        migrated[i] = migrated[i] with { Blocked = reason is not null, BlockedReason = reason };
                        if (reason is not null)
                        {
                            logger.LogWarning(new EventId(1404, "RepositoryQuarantined"),
                                "Quarantined repository {RepositoryId} at startup because it failed validation: {QuarantineReason}",
                                migrated[i].Id, reason);
                        }
                    }
                    return migrated;
                }
    ...
    /// <summary>Returns a human-readable quarantine reason, or null when the persisted entry is still usable.</summary>
    private string? ValidatePersistedEntry(RepositoryRegistration entry)
    {
