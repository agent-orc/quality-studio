...
public sealed class RepositoryRegistry
{
        ...
        if (File.Exists(registryPath))
        {
            try
            {
                ...
                if (loaded is { Count: > 0 })
                {
                    ...
                    {
                        var quarantineReason = DescribeQuarantineReason(migrated[index]);
                        if (quarantineReason is null) continue;
                        logger.LogWarning(new EventId(1404, "RepositoryQuarantined"),
                            "Quarantined repository {RepositoryId} at boot; it will be excluded from use until fixed: {QuarantineReason}",
                            migrated[index].Id, quarantineReason);
                        migrated[index] = migrated[index] with { Blocked = true, BlockedReason = quarantineReason };
                    }
                    return migrated;
                }
            }
    ...
    /// <summary>Non-throwing check used at boot: a bad persisted entry is quarantined, not fatal.</summary>
    private string? DescribeQuarantineReason(RepositoryRegistration entry)
    {
        if (!Directory.Exists(entry.RootPath))
            return $"Registered repository root does not exist: '{entry.RootPath}'.";
        var reason = TryDescribeAllowedDirectoryViolation(entry.RootPath,
        ...
        if (entry.GlobalInputsDirectory is not null)
        {
            ...
            reason = TryDescribeAllowedDirectoryViolation(entry.GlobalInputsDirectory,
    ...
    private string? TryDescribeAllowedDirectoryViolation(string path, string internalMessage)
    {
        try
        {
            EnsureAllowedDirectory(path, internalMessage);
            return null;
        }
    ...
    private void EnsureAllowedDirectory(string path, string internalMessage)
    {
        ...
        var detail = $"{internalMessage} Path: '{path}'. Allowed roots: [{string.Join(", ", allowedRoots)}].";
        if (allowedRoot is null)
            throw new RepositoryRegistryValidationException(detail,
