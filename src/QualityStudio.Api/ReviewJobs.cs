    ...
    ILoggerFactory loggerFactory) : IReviewExecutorFactory
{
    ...
    private readonly CliRunner runner = new(
        // Review prompts routinely exceed OS process argument limits. Stdin keeps the
        // full prompt out of argv and lets a Claude child attach before review work begins.
...
public sealed class ReviewJobService : BackgroundService
{
        ...
        ReviewModelCatalog modelCatalog)
    {
        ...
        ValidateWatchdogOptions(this.options);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var stopwatch = Stopwatch.StartNew();
        var operationLimit = options.OperationTimeout;
        ...
        {
            var attachLimit = options.ReviewerAttachTimeout;
        ...
        string operation)
    {
        ...
        var reclaimed = await Task.WhenAny(execution,
            Task.Delay(options.ReclaimGracePeriod, CancellationToken.None)).ConfigureAwait(false) == execution;
        if (!reclaimed)
        {
            ...
            logger.LogCritical(new EventId(1513, "ReviewerReclaimTimedOut"),
                ...
                operation, reviewRunId, options.ReclaimGracePeriod, stopRequested);
        }
    ...
    private ReviewOperationException OperationTimeout(
        ...
        $"Reviewer operation exceeded watchdog policy v{ReviewJobsOptions.WatchdogPolicyVersion} wall-clock limit {options.OperationTimeout}.",
        operation,
        true,
        diagnostics?.RunId,
        diagnostics?.LogDirectory,
        ...
        string operation)
    {
        ...
        return new ReviewOperationException(new ReviewRunError(
            ...
            diagnostics?.LogDirectory,
            diagnostics?.CliPath), exception);
    }

    private static void ValidateWatchdogOptions(ReviewJobsOptions configured)
    {
        if (configured.ReviewerAttachTimeout <= TimeSpan.Zero)
            throw new InvalidOperationException("ReviewJobs:ReviewerAttachTimeout must be positive.");
        if (configured.OperationTimeout <= TimeSpan.Zero)
            throw new InvalidOperationException("ReviewJobs:OperationTimeout must be positive.");
        if (configured.ReclaimGracePeriod <= TimeSpan.Zero)
            throw new InvalidOperationException("ReviewJobs:ReclaimGracePeriod must be positive.");
    }
