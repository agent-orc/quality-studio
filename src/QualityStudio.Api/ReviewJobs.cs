...
public sealed class ReviewJobService : BackgroundService
{
    ...
    private static IReadOnlyList<string> EnumeratePreflightInputs(string root)
    {
        ...
        while (pending.TryPop(out var directory))
        {
            files.AddRange(Directory.EnumerateFiles(directory, "*", SearchOption.TopDirectoryOnly)
                .Where(path => !IsReparsePoint(path) && IsPreflightInput(root, path)));
            foreach (var child in Directory.EnumerateDirectories(directory, "*", SearchOption.TopDirectoryOnly)
                         .OrderByDescending(path => path, StringComparer.Ordinal))
            {
                if (!IsReparsePoint(child) &&
                    Path.GetFileName(child) is not (".git" or ".quality" or "bin" or "obj" or "node_modules" or "dist" or "out-tsc"))
                    pending.Push(child);
            }
        }
        ...
        foreach (var area in new[] { "security", "static-analysis" })
        {
            ...
            files.AddRange(Directory.EnumerateFiles(directory, "*", new EnumerationOptions
                {
    ...
    private IReadOnlyList<ReviewSensorConfiguration> EnabledConfigurations(
        RepositoryRegistration registration) => (registration.Sensors ?? [])
        .Where(sensor => sensor.Enabled && IsPreflightSensor(sensor.Id))
    ...
    private bool IsPreflightSensor(string id)
    {
        var sensor = sensorRegistry.Get(id);
        return sensor is IDeterministicEvidenceSensor or ISecurityEvidenceSensor;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var collector = new PreflightCollector(sensorRegistry);
            var configurationHash = collector.ConfigurationSetHash(configurations);
            var toolIdentity = await collector.ProbeToolsAsync(configurations, cancellationToken)
                .ConfigureAwait(false);
            ...
            if (existing is not null &&
                ...
                string.Equals(existing.ToolHash, toolIdentity.Hash, StringComparison.Ordinal))
            {
            ...
            try
            {
                var snapshot = await collector.CollectAsync(
                    ...
                    configurations,
                    cancellationToken,
                    toolIdentity).ConfigureAwait(false);
    ...
    private sealed class ReviewWorkItem
    {
        ...
        public static ReviewWorkItem Create(
            ...
            RepositoryRegistration repository,
            ReviewRunStore store,
            PreflightSnapshot? preflight = null) => new(manifest, repository, store, null, null, null, preflight);
