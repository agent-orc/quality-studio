...
public sealed class ReviewJobService : BackgroundService
{
    ...
    private async Task RunAsync(ReviewWorkItem item, CancellationToken stoppingToken)
    {
        ...
        // Defense in depth: cancelling attemptToken normally makes the attempt above
        // unwind on its own — every reviewer CLI operation is already bounded (see
        // CodingAgentReviewAgent). But the single-reader queue must never wedge behind
        // one stuck attempt even if that bound is ever broken, so a cancelled attempt
        // gets a grace period to finish; past that, the queue stops waiting on it and
        // advances. ReviewWorkItem's state guards make a late completion callback from
        // the abandoned attempt a harmless no-op.
