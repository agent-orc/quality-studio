...
app.MapPost("/api/repos/{repoId}/migrate-quality-data", (string repoId, RepositoryRegistry registry) =>
    Results.Ok(registry.MigrateData(repoId)));
...
static IResult ScopeRules(HttpContext context, RepositoryRegistry registry, RepositoryHierarchyCache hierarchyCache)
{
    ...
    return Results.Ok(ScopeRuleResponse(store.Read(), ScopeCandidateFiles(repository.Root, hierarchyCache, repository.DataRoot), store));
}
    ...
    RepositoryRegistry registry, RepositoryHierarchyCache hierarchyCache)
{
    ...
    var preview = store.Preview(new RepositoryScopeRule(request.Action, request.Pattern, request.Reason),
        ScopeCandidateFiles(repository.Root, hierarchyCache, repository.DataRoot));
    ...
    RepositoryRegistry registry, RepositoryHierarchyCache hierarchyCache)
{
    var (_, repository) = ResolveRepository(context, registry);
    var store = new RepositoryScopeConfigurationStore(repository.DataRoot);
    var candidates = ScopeCandidateFiles(repository.Root, hierarchyCache, repository.DataRoot);
...
static IReadOnlyList<string> ScopeCandidateFiles(string repositoryRoot, RepositoryHierarchyCache hierarchyCache, string? dataRoot = null) =>
    Flatten(hierarchyCache.Get(repositoryRoot, projectDataRoot: dataRoot).Roots)
    ...
    RepositoryHierarchyCache hierarchyCache, ILogger<Program> logger, CancellationToken cancellationToken)
{
    ...
    _ = hierarchyCache.Get(repository.Root, globalInputsDirectory: globalDirectory,
        inputBudgetCharacters: registration.InputBudgetCharacters,
        projectDataRoot: repository.DataRoot);
