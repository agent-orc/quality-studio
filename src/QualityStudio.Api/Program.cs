...
static IResult Inputs(HttpContext context, RepositoryRegistry registry, InputResolver resolver, ILogger<Program> logger)
{
    ...
    var kinds = registration.EnabledReviewKinds.ToDictionary(
        kind => kind,
        kind => resolver.Resolve(repository.DataRoot, kind, ReviewLevel.File,
...
{
    var repository = registry.Get(RouteRepositoryId(context));
    var report = new QualityRunReportStore(repository.DataRoot!).Load(id);
    ...
    RepositoryRegistry registry)
{
    ...
    var repository = registry.Get(RouteRepositoryId(context));
    var reports = new QualityRunReportStore(repository.DataRoot!).LoadAll();
...
static IResult ReviewRunCompare(HttpContext context, string baselineId, string candidateId, RepositoryRegistry registry)
{
    ...
    var repository = registry.Get(RouteRepositoryId(context));
    var store = new QualityRunReportStore(repository.DataRoot!);
...
{
    var repository = registry.Get(RouteRepositoryId(context));
    var size = new QualityRunReportStore(repository.DataRoot!).MeasureSize();
    var pinned = new QualityRunReportPinStore(repository.DataRoot!).Load();
...
static IResult PinReviewRun(HttpContext context, string id, RepositoryRegistry registry)
{
    ...
    var pinned = new QualityRunReportPinStore(repository.DataRoot!).Pin(id);
    return Results.Ok(new ReviewRunPinsResponse(pinned.Order(StringComparer.Ordinal).ToArray()));
}
...
{
    var repository = registry.Get(RouteRepositoryId(context));
    var pinned = new QualityRunReportPinStore(repository.DataRoot!).Unpin(id);
    return Results.Ok(new ReviewRunPinsResponse(pinned.Order(StringComparer.Ordinal).ToArray()));
}
