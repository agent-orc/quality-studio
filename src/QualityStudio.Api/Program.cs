...
builder.Services.AddSingleton<ProjectDataRoot>();
...
static IResult Inputs(HttpContext context, RepositoryRegistry registry, InputResolver resolver, ILogger<Program> logger)
{
    ...
    var kinds = registration.EnabledReviewKinds.ToDictionary(
        ...
        kind => resolver.Resolve(repository.Root, kind, ReviewLevel.File,
            globalDirectory, registration.InputBudgetCharacters, dataRoot: repository.DataRoot),
...
static IResult Rules(HttpContext context, string? kind, string? adapter, RepositoryRegistry registry)
{
    ...
    var catalogue = new RuleCatalogueResolver().Resolve(repository.DataRoot, globalDirectory);
    var projectOverridePath = Path.GetFullPath(Path.Combine(repository.DataRoot,
    ...
    ILogger<Program> logger, CancellationToken cancellationToken)
{
    var stopwatch = Stopwatch.StartNew();
    var (registration, repository) = ResolveRepository(context, registry);
    var result = await scanner.ScanAsync(new SecurityScanRequest(
        repository.Root, PersistMetadata: false, DataRoot: repository.DataRoot), cancellationToken);
