...
public sealed class ReviewRunArchiveStore
{
    ...
    private void ValidateOperation(ReviewRunOperationRecord operation)
    {
        ValidateRunId(operation.RunId);
        if (!IsId(operation.OperationId) || operation.Ordinal < 1 || operation.Attempt < 1 ||
            !IsId(operation.UnitId) || !IsRepositoryPath(operation.Path) ||
            ...
            operation.ResultSidecar is not null && !IsRepositoryPath(operation.ResultSidecar) ||
    ...
    private void ValidateFinding(ReviewRunFindingRecord finding)
    {
        ...
        if (!IsId(finding.OperationId) || !IsFingerprint(finding.Fingerprint) ||
            ...
            finding.Locations.Any(location => location is null || !IsRepositoryPath(location.Path)))
            throw new ArgumentException("The archive finding record is invalid.", nameof(finding));
    }

    private void ValidateRun(ReviewRunArchiveRecord run)
    {
        ...
        if (!IsId(run.RepositoryId) || run.CreatedAt.Offset != TimeSpan.Zero || run.Subject is null ||
            ...
            !IsRepositoryPath(run.Subject.Path) || !Levels.Contains(run.Level) || !Kinds.Contains(run.Kind) ||
            run.Targets is null || run.Targets.Count == 0 || run.Targets.Any(target => target is null ||
                !IsId(target.Id) || string.IsNullOrWhiteSpace(target.Name) || !IsRepositoryPath(target.Path) ||
    ...
    private bool IsRepositoryPath(string? path)
    {
        if (string.IsNullOrWhiteSpace(path) || Path.IsPathRooted(path) || path.Contains('\\')) return false;
        var absolute = Path.GetFullPath(Path.Combine(repositoryRoot,
            path.Replace('/', Path.DirectorySeparatorChar)));
        if (!PathConfinement.IsWithin(repositoryRoot, absolute)) return false;
        PathConfinement.RejectReparseTraversal(repositoryRoot, absolute);
        var normalized = Path.GetRelativePath(repositoryRoot, absolute).Replace('\\', '/');
