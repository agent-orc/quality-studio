...
- Real 3,927-file startup is live at 983.84 ms median but not usable until 12.310 s median because the first request waits behind cold prewarming.
- The warm root tree is 29,119,333 bytes and project-plus-tree takes 1,105.95 ms median / 1,846.68 ms p95, missing the 500 ms p95 contract.
- One live review took 17.327 s; the model call was 16.752 s (96.68%). Strict parsing was 0.2162 ms median.
- Review terminal state took 1,709.14 ms median to become visible because of polling; terminal-response-to-visible was 291.51 ms in the current refresh path.
- Anonymous RSS retained 17,572 KiB (+30.25%) after 100 Git-state invalidations, consistent with the unbounded dashboard cache.
...
- Routine core tests: 155 passed; one opt-in live integration test skipped. The separate machine-bound 5,000-file hierarchy test passed.
- Routine API tests: 47 passed. The separate machine-bound 5,000-file projection test failed at 180 ms against its single-sample 150 ms timing assertion under the measured host load.
- Angular unit tests: 39 passed using the cached Chromium launcher with the repository-supported no-sandbox mode.
- Browser interaction harness: dashboard 93.9 ms, tree 4.9–25.1 ms, file 51.0 ms, aspect switch 12.7 ms; all budgets passed.
- Real-backend switch harness: transitions 11.7–21.3 ms and usable state 32.6–196.6 ms; all three switch budgets passed.
